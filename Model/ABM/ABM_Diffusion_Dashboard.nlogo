...
to setup
  ...
  if not is-number? interaktivitaet [ set interaktivitaet 0.0 ]
  if not is-number? standortattraktivitaet [ set standortattraktivitaet 0.0 ]
...
to apply-environment-interventions
  ...
  if interaktivitaet > 0 [
    let function-boost interaktivitaet * 1.0 * saturation-factor
  ...
  if standortattraktivitaet > 0 [
    let compatibility-boost standortattraktivitaet * 0.6 * saturation-factor
...
to export-results
  ...
  print (word empfehlung "," informationskampagnen "," interaktivitaet ","
             standortattraktivitaet "," total-adopters "," adopter-c1 "," adopter-c2 ","
...
to clear-values
  ...
  set interaktivitaet 0
  set standortattraktivitaet 0
end
@#$#@#$#@
GRAPHICS-WINDOW
302
